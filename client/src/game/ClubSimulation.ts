...
export class ClubSimulation {
  ...
  private logs = [`監督就任。${this.clubName}の新しいシーズンが始まる。`];
  ...
  private resetToInitialState() {
    ...
    this.rows = baseRows();
    this.logs = [`監督就任。${this.clubName}の新しいシーズンが始まる。`];
  ...
  get selectedMarketCandidateIdValue() { return this.currentMarketCandidate?.id ?? null; }
  ...
  setClubName(value: string) {
    ...
    if (!nextName || nextName === this.clubName) return { ok: false, text: "新しいチーム名を入力してください。" };
    const previousName = this.clubName;
    this.clubName = nextName;
    this.rows = this.rows.map((row) => row.id === userClub.id ? { ...row, name: nextName } : row);
  ...
  setAutoLineupCriteria(criteria: AutoLineupCriteria) {
    if (!["attack", "defense", "fit"].includes(criteria)) return;
    this.autoLineupCriteria = criteria;
    this.persist();
  }
  ...
  private autoLineupScore(player: Player, slotId: string, criteria: AutoLineupCriteria) {
    const slot = this.formation.slots.find((item) => item.id === slotId);
    ...
    const fitBonus = primaryFit ? 14 : secondaryFit ? 6 : 0;
    const ability = criteria === "attack" ? this.attackPower(player) : criteria === "defense" ? this.defensePower(player) : this.playerPower(player, slot?.label ?? player.position);
    return ability + (criteria === "fit" ? fitBonus : (primaryFit ? 5 : secondaryFit ? 2 : 0)) - fatiguePenalty;
  }

  autoLineup(criteria: AutoLineupCriteria = this.autoLineupCriteria) {
    this.autoLineupCriteria = criteria;
    ...
    for (const slot of this.formation.slots) {
      ...
      const player = [...pool].sort((a, b) => this.autoLineupScore(b, slot.id, criteria) - this.autoLineupScore(a, slot.id, criteria))[0];
      if (player) {
  ...
  selectMarketCandidate(candidateId: string) {
    const candidate = this.marketCandidates.find((player) => player.id === candidateId);
    if (!candidate) return { ok: false, text: "この候補は現在の市場リストにありません。" };
    this.selectedMarketCandidateId = candidate.id;
    this.persist();
    return { ok: true, text: `${candidate.name}のスカウトデータを表示しました。` };
  }
  ...
  private createMatchFlow(playerGoals: number, opponentGoals: number, opponentName: string, tactics: TacticalAssessment, opponentTactics: OpponentTacticalAssessment, tacticalMatchup: TacticalMatchup, matchCondition: TeamMatchCondition, matchWeek = this.week, fixedHalf?: Pick<HalfTimeReport, "playerGoals" | "opponentGoals">): { halfTime: HalfTimeReport; highlights: MatchHighlight[] } {
    ...
    highlights.push({ minute: 90, kind: "fulltime", team: "neutral", text: `試合終了。${this.clubName} ${playerGoals}-${opponentGoals} ${opponentName}。` });
  ...
  private createPlayerRatings(highlights: MatchHighlight[], substitutions: MatchSubstitution[], injuries: MatchInjury[]): PlayerMatchRating[] {
    ...
    return participants.map((player) => {
      ...
      const commentGroups = injured
        ...
        : rating < 5.9
          ? ["プレーの判断が遅れ、持ち味を出せなかった", "相手の圧力に苦しみ、ミスが目立った", "攻守で後手に回り、流れを変えられなかった", "ボールロストが重なり、チームに負担をかけた", "対人局面で競り負ける場面が多かった", "ポジショニングが乱れ、危険なスペースを空けた", "期待された役割を十分に果たせない試合だった", "終盤に運動量が落ち、プレーの精度を欠いた", "判断に迷いが見られ、攻撃の停滞を招いた", "守備への戻りが遅れ、相手に主導権を渡した"]
          : rating < 6.6
            ? ["奮闘したが、決定的な仕事には届かなかった", "要所で対応した一方、安定感を欠いた", "局面によっては良さを見せたが、波のある内容だった", "役割はこなしたものの、もう一段の上積みが欲しい", "チームの流れに乗り切れず、存在感を出し切れなかった"]
            : goals
