...
export class GameUI {
  ...
  private teamPowerRadarCard() {
    ...
    const labelMap: Record<typeof radar.strengthKey, string> = { attack: "攻撃", defense: "守備", midfield: "中盤", chemistry: "連携", tacticalAdaptation: "戦術適応", stability: "安定性" };
    const aria = values.map((item) => `${item.label}${item.value}`).join("、");
    return `<article class="tactical-card team-power-card"><div class="team-power-head"><div><span class="card-kicker">TEAM PROFILE</span><h3>チーム力 <small>6項目</small></h3></div><strong>${radar.overall}<small>PROFILE</small></strong></div><div class="team-power-chart-wrap"><svg class="team-power-radar" viewBox="0 0 260 220" role="img" aria-label="チーム力 ${aria}"><g class="team-power-grid">${[20, 40, 60, 80, 100].map((level) => `<polygon points="${polygon(level)}"></polygon>`).join("")}</g>${items.map((_, index) => `<line x1="130" y1="102" x2="${point(index, 100).split(",")[0]}" y2="${point(index, 100).split(",")[1]}"></line>`).join("")}<polygon class="team-power-area" points="${values.map((item, index) => point(index, item.value)).join(" ")}"></polygon>${values.map((item) => `<text x="${item.x.toFixed(1)}" y="${item.y.toFixed(1)}" text-anchor="middle" dominant-baseline="middle" class="${item.value >= 80 ? "is-strong" : item.value < 60 ? "is-weak" : ""}">${item.short} ${item.value}</text>`).join("")}</svg></div><div class="team-power-legend">${values.map((item) => `<div><span>${item.label}</span><b class="${item.value >= 80 ? "is-strong" : item.value < 60 ? "is-weak" : ""}">${item.value}</b></div>`).join("")}</div><p class="team-power-diagnosis"><b>強み</b> ${labelMap[radar.strengthKey]} <strong>${radar[radar.strengthKey]}</strong><span>課題 ${labelMap[radar.weaknessKey]} <strong>${radar[radar.weaknessKey]}</strong></span></p></article>`;
  }
