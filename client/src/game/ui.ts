...
export class GameUI {
  ...
  private handleClick = (event: MouseEvent) => {
    ...
    switch (target.dataset.action) {
      ...
      case "close-modal": this.stopLiveCommentary(); this.stopGoalCelebration(); this.modal = false; this.matchStage = "fulltime"; this.resetHalfTimeControls(); this.render(); break;
  ...
  private marketPage() {
    ...
    const budget = this.simulation.marketBudgetSimulation;
    ...
    if (!candidate || !negotiation) return `${this.pageHeading("TRANSFER WINDOW", "市場を待つ", "次のスカウトサイクルを待ち、候補が届くのを待つ。")}${capacityAlert}<section class="tactical-card market-empty"><span class="card-kicker">SCOUTING CYCLE</span><h2>全候補と交渉済み</h2><p>新たなシーズンに、次の市場候補が届きます。</p></section>`;
    ...
    return `
      ...
      <section class="market-tools-grid"><article class="candidate-comparison tactical-card"><div class="market-tool-head"><div><span class="card-kicker">CANDIDATE COMPARISON</span><h3>候補比較リスト</h3></div><b>${comparisons.length} PROFILES</b></div><div class="candidate-compare-head"><span>候補</span><span>OF / DF</span><span>戦術</span><span>優先</span><span>移籍金</span></div>${comparisons.map((item) => `<div class="candidate-compare-row ${item.status === "閲覧中" ? "is-current" : ""}"><strong><i>${item.status === "閲覧中" ? "NOW" : ""}</i>${item.player.name}<small>${positionLabel(item.player.position)}${item.player.secondary ? ` / ${positionLabel(item.player.secondary)}` : ""}　${item.player.age}歳</small></strong><span>${item.player.position === "GK" ? `GK ${item.player.gk}` : `OF ${item.player.attack}`}<small>DF ${item.player.defense}</small></span><span class="compare-fit">${item.tacticalFit.score}<small>${item.tacticalFit.grade}</small></span><span class="compare-priority ${item.recruitmentPriority.grade === "最優先" ? "urgent" : ""}">${positionLabel(item.recruitmentPriority.position)} ${item.recruitmentPriority.score}<small>${item.recruitmentPriority.grade}</small></span><span>${formatMoney(item.openingFee)}<small>年俸 ${formatMoney(item.annualImpact)}</small></span></div>`).join("")}</article><article class="budget-simulation tactical-card"><div class="market-tool-head"><div><span class="card-kicker">RECRUITMENT BUDGET</span><h3>補強予算シミュレーター</h3></div><b class="${budget.canCoverReserve ? "is-safe" : "is-risk"}">${budget.canCoverReserve ? "SAFE" : "CAUTION"}</b></div><p>${budget.candidate ? `${budget.candidate.name}を現在の条件で獲得した場合の資金見通しです。` : "市場候補を待っています。"}</p><div class="budget-rows"><span>手元資金<b>${formatMoney(budget.cashBefore)}</b></span><span>想定移籍金<b>−${formatMoney(budget.transferFee)}</b></span><span>年俸増額 / 週<b>−${formatMoney(budget.weeklySalaryImpact)}</b></span><span>契約更新の確保額<b>−${formatMoney(budget.renewalReserve)}</b></span></div><div class="budget-result"><span>移籍後の残高<b class="${budget.canAffordTransfer ? "is-safe" : "is-risk"}">${formatMoney(budget.cashAfterTransfer)}</b></span><span>更新確保後<b class="${budget.canCoverReserve ? "is-safe" : "is-risk"}">${formatMoney(budget.cashAfterReserve)}</b></span></div></article></section>
  ...
  private highlightsTimeline(highlights: NonNullable<ReturnType<ClubSimulation["advanceWeek"]>>["highlights"], throughHalf: boolean, visibleCount?: number) {
    ...
    const line = (item: MatchHighlight) => `<article class="highlight-item ${item.team} ${item.kind}"><b>${String(item.minute).padStart(2, "0")}′</b><span>${item.kind === "goal" ? "GOAL" : item.kind === "injury" ? "MED" : item.kind === "tactic" ? "PLAN" : item.kind === "halftime" ? "HT" : "•"}</span><p>${item.text}</p></article>`;
