...
export class GameUI {
  ...
  private trainingPage(score: ReturnType<ClubSimulation["score"]>) {
    ...
    return `
      ...
      <section class="training-menu">${trainingOptions.filter((option) => option.id !== "recovery").map((option) => { const risk = this.simulation.trainingRiskReport(option.id); const recommended = option.id === recommendation.focus; return `<article class="training-option tactical-card ${option.accent} ${!canTrain ? "is-complete" : ""} ${recommended ? "is-recommended" : ""}"><span class="card-kicker">${!canTrain ? "WEEKLY LIMIT" : recommended ? "RECOMMENDED" : "ABILITY PROGRAM"}</span><h3>${option.label}</h3><p>${!canTrain ? "今週の練習枠を使い切りました。次節後に再開します。" : option.copy}</p><div><span>実施費用</span><b>${!canTrain ? "完了" : formatMoney(option.cost)}</b></div><small class="training-risk ${risk.grade === "高" ? "is-high" : risk.grade === "注意" ? "is-caution" : ""}">負傷リスク ${risk.chance}% / ${risk.grade}　${risk.note}</small><button data-action="train" data-training="${option.id}" class="${option.accent === "coral" ? "ghost-action" : "primary-action"}" ${!canTrain ? "disabled" : ""}>${!canTrain ? "今週は実施済み" : "このメニューを実施"} <b>${!canTrain ? "✓" : "↗"}</b></button></article>`; }).join("")}</section>
