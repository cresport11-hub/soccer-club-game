...
export class GameUI {
  ...
  private playerDetailPanel() {
    ...
    return `<div class="player-detail-overlay" role="dialog" aria-modal="true" aria-label="${player.name}の選手詳細"><section class="player-detail-panel"><header><div><span>PLAYER PROFILE</span><h2>${player.name}</h2><p>${role}</p></div><button type="button" data-close-player-detail aria-label="選手詳細を閉じる">×</button></header><div class="detail-identity"><span class="pos-tag">${positionLabel(player.position)}</span><div><b>${player.age}<small>歳</small></b><p>年齢</p></div><div><b>LV.${player.level}<small> / ${player.ceiling}</small></b><p>現在レベル / 上限</p></div></div><section class="detail-ability"><h3>OFFENSE / 攻撃能力</h3><div><span>OF</span><b>${player.attack}</b><i><em style="width:${player.attack}%"></em></i></div><div><span>ドリブル</span><b>${player.dribble}</b><i><em style="width:${player.dribble}%"></em></i></div><div><span>パス</span><b>${player.pass}</b><i><em style="width:${player.pass}%"></em></i></div><div><span>シュート</span><b>${player.shoot}</b><i><em style="width:${player.shoot}%"></em></i></div><h3 class="defense-heading">DEFENSE / 守備能力</h3><div><span>DF</span><b>${player.defense}</b><i class="blue"><em style="width:${player.defense}%"></em></i></div><div><span>タックル</span><b>${player.tackle}</b><i class="blue"><em style="width:${player.tackle}%"></em></i></div><div><span>ブロック</span><b>${player.block}</b><i class="blue"><em style="width:${player.block}%"></em></i></div><div><span>パスカット</span><b>${player.interception}</b><i class="blue"><em style="width:${player.interception}%"></em></i></div>${player.position === "GK" ? `<h3 class="keeper-heading">GOALKEEPING</h3><div><span>GK</span><b>${player.gk ?? 0}</b><i class="gold"><em style="width:${player.gk ?? 0}%"></em></i></div>` : ""}</section>${growthPanel}${cfStyleControl}<section class="detail-note"><span>CONDITION / CONTRACT</span><b class="detail-condition-status ${conditionStatus.tone}">${condition}</b><i class="detail-condition-meter"><em style="width:${conditionStatus.value}%"></em></i><small>${conditionStatus.note}</small><p>年俸 ${formatMoney(player.salary)}　/　勝利 ${player.winBonus ? formatMoney(player.winBonus) : "なし"}　/　出場 ${player.appearanceBonus ? formatMoney(player.appearanceBonus) : "なし"}　/　得点 ${player.goalBonus ? formatMoney(player.goalBonus) : "なし"}　/　残り ${player.contractYears ?? 1}年</p></section><button type="button" class="detail-close-button" data-close-player-detail>選手一覧へ戻る</button></section></div>`;
  }
