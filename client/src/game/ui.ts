...
export class GameUI {
  ...
  private handleClick = (event: MouseEvent) => {
    ...
    if (target.dataset.rosterSalaryFilter) { this.rosterSalaryFilter = target.dataset.rosterSalaryFilter as SalaryFilter; this.showToast("年俸の並び順を更新しました。"); this.render(); return; }
    ...
    switch (target.dataset.action) {
      ...
      case "set-training-focus": { const result = this.simulation.setTrainingFocus(target.dataset.trainingPlayer ?? "", target.dataset.trainingFocus as TrainingFocus); this.showToast(result.text); this.render(); break; }
      ...
      case "negotiate-recruit": { const result = this.simulation.negotiateRecruit((target.dataset.offerTier as "low" | "fair" | "high") ?? "fair"); this.showToast(result.text); this.render(); break; }
      case "accept-recruit-hold": { const result = this.simulation.acceptRecruitHold(); this.showToast(result.text); this.render(); break; }
      case "adjust-recruit-hold": { const result = this.simulation.adjustRecruitHold(); this.showToast(result.text); this.render(); break; }
      case "withdraw-recruit": { const result = this.simulation.withdrawRecruitNegotiation(); this.showToast(result.text); this.render(); break; }
  ...
  private markingForOpponent(opponentPlayer: OpponentPlayer, opponentSlot: Slot): MarkingAssessment | null {
    if (!isMarkableOpponentPosition(opponentPlayer.position)) return null;
    const candidates = this.simulation.formation.slots.map((slot) => ({ player: this.simulation.playerForSlot(slot.id), slot })).filter((item): item is { player: Player; slot: Slot } => item.player !== null && isMarkingDefenderPosition(item.player.position));
    if (!candidates.length) return null;
    const opponentFormation = formations.find((formation) => formation.id === this.simulation.currentOpponentTactics.formationId);
    const markableOpponents = this.simulation.currentOpponentTactics.lineup.filter((player) => isMarkableOpponentPosition(player.position));
    const remaining = [...candidates];
    let pairing: { player: Player; slot: Slot } | undefined;
    let manual = false;
    // シミュレーションと同じ順番で割り当て、守備担当1人につき相手1人だけにする。
    for (const target of markableOpponents) {
      if (!remaining.length) break;
      const targetSlot = opponentFormation?.slots.find((slot) => target.id.endsWith(`-${slot.id}`)) ?? opponentFormation?.slots[0] ?? opponentSlot;
      const manualPlayerId = this.simulation.currentManualMarkAssignments[target.id];
      const manualIndex = remaining.findIndex((candidate) => candidate.player.id === manualPlayerId && canMarkOpponent(target.position, candidate.player.position));
      const bestIndex = manualIndex >= 0 ? manualIndex : remaining.reduce((best, candidate, index) => {
        const candidateRank = markingDefenderRank(target.position, candidate.player.position) ?? 99;
        const bestRank = markingDefenderRank(target.position, remaining[best].player.position) ?? 99;
        const candidateScore = candidateRank * 10000 + (candidate.slot.x - targetSlot.x) ** 2 + (candidate.slot.y - targetSlot.y) ** 2;
        const bestScore = bestRank * 10000 + (remaining[best].slot.x - targetSlot.x) ** 2 + (remaining[best].slot.y - targetSlot.y) ** 2;
        return candidateScore < bestScore ? index : best;
      }, 0);
      const selected = remaining[bestIndex];
      remaining.splice(bestIndex, 1);
      if (target.id === opponentPlayer.id) {
        pairing = selected;
        manual = selected.player.id === manualPlayerId;
        break;
      }
    }
    ...
    const distance = Math.hypot(pairing.slot.x - opponentSlot.x);
    ...
    const reason = tone === "advantage" ? `${pairing.player.name}（${positionLabel(pairing.player.position)}）の守備・奪取力が上回る。` : tone === "caution" ? `${opponentPlayer.role}の前進力に注意。${pairing.player.name}の背後をカバーしたい。` : `守備担当 ${pairing.player.name}との能力差は小さい。受ける前の圧力で主導権を取る。`;
    return { player: pairing.player, tone, label, differential, reason, manual };
  }
  ...
  private decorateManualMarkControls() {
    ...
    const starters = this.simulation.formation.slots.map((slot) => ({ slot, player: this.simulation.playerForSlot(slot.id) })).filter((item): item is { slot: Slot; player: Player } => item.player !== null && isMarkingDefenderPosition(item.player.position));
    const assignedPlayerIds = new Set(Object.values(manualAssignments));
    content.insertAdjacentHTML("beforebegin", `<section class="manual-mark-console"><header><div><span>MANUAL MARKING</span><b>守備担当を手動指定</b></div><div><i>${Object.keys(manualAssignments).length} / ${starters.length} 指定</i><button type="button" data-mark-clear ${Object.keys(manualAssignments).length ? "" : "disabled"}>自動へ戻す</button></div></header><p>CB・SB・DH・CH・SHの選手札を<strong>相手の前線・中盤の攻撃選手へ1人1名ずつドラッグ</strong>してください。GK・CB・SBは個別マーク対象外、守備担当が足りない相手はゾーン対応です。スマホでは、選手札をタップしてから相手をタップします。</p><div class="manual-mark-sources">${starters.map(({ slot, player }) => `<button type="button" draggable="true" data-mark-source="${player.id}" class="manual-mark-source ${this.manualMarkSourceId === player.id ? "is-selected" : ""} ${assignedPlayerIds.has(player.id) ? "is-assigned" : ""}" aria-pressed="${this.manualMarkSourceId === player.id}" aria-label="${player.name}を守備担当に選択、またはドラッグ"><i>${positionLabel(slot.label)}</i><b>${player.name}</b><small>${assignedPlayerIds.has(player.id) ? "担当済" : "ドラッグ"}</small></button>`).join("")}</div></section>`);
  }
  ...
  private decorateOpponentMarkings() {
    ...
    this.root.querySelectorAll<HTMLButtonElement>(".opponent-token[data-opponent-player]").forEach((token) => {
      const opponentPlayer = opponent.lineup.find((player) => player.id === token.dataset.opponentPlayer);
      const assessment = assessmentFor(token.dataset.opponentPlayer ?? "");
      if (!opponentPlayer) return;
      if (!isMarkableOpponentPosition(opponentPlayer.position)) {
        token.classList.add("mark-not-required");
        token.dataset.mark = "個別マーク対象外";
        token.insertAdjacentHTML("beforeend", `<em class="mark-player-label not-required">個別マーク対象外 / ゾーン対応</em>`);
        token.setAttribute("aria-label", `${token.getAttribute("aria-label") ?? "相手選手"}。${positionLabel(opponentPlayer.position)}のため個別マークは設定しません。守備ブロックで対応します。`);
        return;
      }
      ...
      token.setAttribute("aria-label", `${token.getAttribute("aria-label") ?? "相手選手"}。${assessment.player.name}（${positionLabel(assessment.player.position)}）との${assessment.manual ? "手動" : "自動"}マーク相性は${assessment.label}。${assessment.reason}`);
    });
    const pitchHead = this.root.querySelector(".opponent-formation-panel .scout-panel-head");
    if (pitchHead) pitchHead.insertAdjacentHTML("afterend", `<div class="marking-legend"><span><i class="advantage"></i>有利</span><span><i class="even"></i>拮抗</span><span><i class="caution"></i>警戒</span><span><i class="not-required"></i>対象外</span><b>攻撃・中盤は一対一（守備1人につき相手1人）、GK・CB・SBと余剰選手はゾーン対応</b></div>`);
  ...
  private decorateMatchSkillSummary() {
    ...
    const conditionGrowth = result.conditionChanges.length ? `状態変化　上昇 ${result.conditionChanges.filter((change) => change.delta > 0).length}名 / 低下 ${result.conditionChanges.filter((change) => change.delta < 0).length}名` : "状態変化　なし";
    plan.insertAdjacentHTML("beforeend", `<br/><strong class="match-condition-summary ${condition.isHome ? "home" : "away"}">${condition.isHome ? "HOME EDGE" : "AWAY PRESSURE"}　攻 ${signed(condition.homeAttack - condition.opponentHomeAttack)} / 守 ${signed(condition.homeDefense - condition.opponentHomeDefense)}</strong><br/><small>${condition.summary}</small><br/><small class="match-condition-growth">${conditionGrowth}</small><br/><small class="match-condition-after">POST MATCH　MORALE ${after.morale} ${after.moraleLabel} / FORM ${signed(after.momentum)} ${after.momentumLabel}</small><br/><strong class="match-skill-summary">SKILL LINK-UP　攻 ${signed(result.tactics.skillAttack)} / 守 ${signed(result.tactics.skillDefense)}</strong><br/><small>${result.tactics.skillSummary}</small><br/><strong class="match-skill-summary opponent">OPPOSITION SKILLS　攻 ${signed(result.opponentTactics.skillAttack)} / 守 ${signed(result.opponentTactics.skillDefense)}</strong><br/><small class="match-skill-growth">${growth}</small>`);
  }
  ...
  private homePage(score: ReturnType<ClubSimulation["score"]>) {
    ...
    const nextClubIncome = nextGate.revenue + nextMerchandise.revenue + nextConcession.revenue + membership.revenue;
    ...
    return `
      ...
      <section class="dashboard-grid">
        ...
        <article class="tactical-card supporter-card home-pulse-card"><div class="card-kicker">CLUB PULSE</div><div class="home-pulse-head"><div><h3>人気と財務</h3><p>次節の集客とクラブ資金の現在値。</p></div><span class="fan-score-compact"><strong>${this.simulation.fanPopularity}<small>%</small></strong><em>${this.simulation.fanPopularity >= 70 ? "熱狂" : this.simulation.fanPopularity >= 45 ? "期待" : "再建中"}</em></span></div><div class="home-pulse-summary"><span><i>所持金</i><b>${formatMoney(this.simulation.currentMoney)}</b></span><span><i>次回見込み収入</i><b>+${formatMoney(nextClubIncome)}</b></span><span><i>次戦</i><b>${nextGate.isHome ? `${nextGate.attendance.toLocaleString()}人` : "AWAY"}</b></span></div><div class="fan-bar"><i style="width:${this.simulation.fanPopularity}%"></i></div><details class="home-pulse-details"><summary><span>収益・人気の詳細</span><small>会員・グッズ・売店・入場料</small><b>＋</b></summary><div class="supporter-commerce"><span><i>FAN CLUB</i><b>${membership.members.toLocaleString()}人</b><small>週次 +${formatMoney(membership.revenue)}</small></span><span><i>GATE ${nextGate.isHome ? "FORECAST" : "AWAY"}</i><b>${nextGate.isHome ? formatMoney(nextGate.revenue) : "—"}</b><small>${nextGate.isHome ? `${nextGate.attendance.toLocaleString()} / ${nextGate.capacity.toLocaleString()}人` : "ホーム戦で入場料"}</small></span><span><i>GOODS</i><b>${nextMerchandise.isHome ? formatMoney(nextMerchandise.revenue) : "—"}</b><small>${nextMerchandise.isHome ? `${nextMerchandise.buyers.toLocaleString()}人が購入予想` : "ホーム戦で販売"}</small></span><span><i>FOOD Lv.${nextConcession.level}</i><b>${nextConcession.isHome ? formatMoney(nextConcession.revenue) : "—"}</b><small>${nextConcession.isHome ? `${nextConcession.customers.toLocaleString()}人が利用予想` : "ホーム戦で営業"}</small></span></div><button data-nav="finance" class="home-pulse-link">財務ダッシュボードを見る <b>→</b></button></details></article>
  ...
  private teamPowerRadarCard() {
    ...
    const items: Array<{ key: keyof Pick<typeof radar, "attack" | "defense" | "midfield" | "chemistry" | "tacticalAdaptation" | "transition">; label: string; short: string }> = [
      ...
      { key: "transition", label: "トランジション", short: "切替" },
    ];
  ...
  private lineupPage(score: ReturnType<ClubSimulation["score"]>, selected: Player | null) {
    ...
    const selectedBaseFormationId = formationBaseId(selectedFormationId);
    ...
    const selectedStructures = formations.filter((formation) => formation.id.startsWith(`${selectedBaseFormationId}-`));
    const baseFormation = baseFormations.find((formation) => formation.id === selectedBaseFormationId);
    const structureMenu = baseFormation ? `<section class="formation-structure-menu"><div><span class="card-kicker">${baseFormation.label} MIDFIELD STRUCTURE</span><small>構成を選択すると、ピッチ上の配置も切り替わります。</small></div><div class="formation-structure-options"><button data-formation="${baseFormation.id}" class="formation-structure-chip ${selectedFormationId === baseFormation.id ? "is-selected" : ""}"><b>基本構成</b><span>${baseFormation.description}</span></button>${selectedStructures.map((formation) => { const [structure, label] = formation.description.split("／"); return `<button data-formation="${formation.id}" class="formation-structure-chip ${selectedFormationId === formation.id ? "is-selected" : ""}"><b>${structure}</b><span>${label ?? "構成"}</span></button>`; }).join("")}</div></section>` : "";
    const formationMenu = `<section class="formation-switcher">${baseFormations.map((formation) => `<button data-formation="${formation.id}" class="formation-chip ${selectedBaseFormationId === formation.id ? "is-selected" : ""}"><b>${formation.label}</b><span>${formation.description}</span></button>`).join("")}</section>${structureMenu}`;
    return `
      ...
      <section class="tactical-console tactical-card tactical-plan-card">
        ...
        <div class="tactical-plan-controls"><div class="tactic-selector"><span class="card-kicker">MENTALITY</span><div class="tactic-options">${mentalityOptions.map((item) => `<button data-mentality="${item.id}" class="tactic-option ${tactics.mentality === item.id ? "is-selected" : ""}"><b>${item.label}</b><small>攻${signed(item.attack)} / 守${signed(item.defense)}</small></button>`).join("")}</div></div><div class="tactic-selector"><span class="card-kicker">PLAYING STYLE</span><div class="tactic-options">${playingStyleOptions.map((item) => `<button data-style="${item.id}" class="tactic-option style ${tactics.playingStyle === item.id ? "is-selected" : ""}"><b>${item.label}</b><small>攻${signed(item.attack)} / 守${signed(item.defense)}</small></button>`).join("")}</div></div></div>
        <details class="advanced-tactics-details"><summary><span>詳細な戦術補正</span><small>役割・スキル・連動の内訳を表示</small><b>＋</b></summary><div class="advanced-tactics-grid"><div class="system-readiness-console"><span class="card-kicker">SYSTEM ADAPTATION</span><div><p>${this.simulation.formation.label}での平均実力発揮率</p><small>システム理解力と、この布陣での試合・練習経験から算出。</small></div><b>${score.systemRate}<small>%</small></b></div><div class="role-fit-console"><span class="card-kicker">ROLE SYNERGY</span><div><p>${tactics.roleFitSummary}</p><small>${roleFitLeaders ? `上位伸長: ${roleFitLeaders}` : "対応位置と役割を揃えると、ここに伸長要因が表示されます。"}</small></div><b>適合伸長　攻 ${signed(tactics.roleFitAttack)} / 守 ${signed(tactics.roleFitDefense)}</b></div><div class="skill-console ${tactics.skillDetails.some((skill) => skill.active) ? "is-active" : ""}"><span class="card-kicker">SKILL LINK-UP</span><div><p>${tactics.skillSummary}</p><small>${skillLeaders || "戦術と能力の発動条件を満たすスキルはありません。"}</small></div><b>発動補正　攻 ${signed(tactics.skillAttack)} / 守 ${signed(tactics.skillDefense)}</b></div><div class="side-link-console ${tactics.sideLinkDetails.length ? "is-active" : ""}"><span class="card-kicker">WIDE LINK-UP</span><div><p>${tactics.sideLinkDetails.length ? tactics.sideLinkSummary : "SH・WGとSBを同じサイドの対応位置へ起用すると、サイド攻略ボーナスが発動します。"}</p><small>${sideLinkLeaders || "左右のレーンに連動ユニットはありません。"}</small></div><b>連動補正　攻 ${signed(tactics.sideLinkAttack)} / 守 ${signed(tactics.sideLinkDefense)}</b></div><div class="midfield-press-console ${tactics.midfieldPressDetail.active ? "is-active" : ""}"><span class="card-kicker">MIDFIELD PRESS</span><div><p>${tactics.midfieldPressSummary}</p><small>${tactics.midfieldPressReason}</small></div><b>連動補正　攻 ${signed(tactics.midfieldPressAttack)} / 守 ${signed(tactics.midfieldPressDefense)}</b></div></div></details>
      </section>
      ${this.opponentDossier()}
      <section class="lineup-layout">
        <article class="tactical-card pitch-panel"><div class="panel-top"><div><span class="card-kicker">TACTICAL BOARD</span><h3>${this.simulation.formation.label} <small>${this.simulation.formation.description}</small></h3></div><button data-action="auto" class="mini-action">⚡ オート編成</button></div><div class="auto-criteria"><span>評価基準</span><div>${([{"id":"attack","label":"攻撃重視"},{"id":"defense","label":"守備重視"},{"id":"fit","label":"適性重視"}] as Array<{id: AutoLineupCriteria; label: string}>).map((option) => `<button type="button" data-auto-criteria="${option.id}" class="criteria-chip ${this.simulation.autoLineupCriteriaValue === option.id ? "is-active" : ""}">${option.label}</button>`).join("")}</div></div>
          ...
          <div class="tactics-pitch" style="background-image:linear-gradient(rgba(4,31,18,.23),rgba(4,31,18,.23)),url('${assets.tacticsBoard}')">
            ${slots.map((slot) => { const player = this.simulation.playerForSlot(slot.id); const fit = player ? this.simulation.playerIsFit(player, slot.id) : true; const injuryWeeks = player ? this.simulation.injuryWeeksFor(player.id) : 0; const conditionStatus = player ? this.simulation.conditionStatusFor(player) : null; const system = player ? this.simulation.systemEffectivenessFor(player) : null; const fatigueState = injuryWeeks ? "is-injured" : player && player.fatigue >= 80 ? "is-danger" : player && player.fatigue >= 60 ? "is-warning" : ""; const isSwapSource = player?.id === selected?.id; const primaryStat = player?.position === "GK" ? `GK${player.gk ?? 0}` : player ? `OF${player.attack}/DF${player.defense}` : ""; const offroleWarning = !fit ? `<em class="offrole-warning" aria-label="ポジション不適性" title="ポジション不適性">⚠</em>` : ""; return `<button data-slot="${slot.id}" class="player-token ${player ? "is-filled" : ""} ${isSwapSource ? "is-swap-source" : ""} ${!fit ? "is-offrole" : ""} ${fatigueState} ${conditionStatus ? `condition-${conditionStatus.tone}` : ""}" aria-label="${player ? `${compactSurname(player.name)} ${positionLabel(slot.label)}${!fit ? "・ポジション不適性" : ""}` : `${positionLabel(slot.label)} 空き`}" style="left:${slot.x}%;top:${slot.y}%"><i>${positionLabel(slot.label)}</i>${offroleWarning}${player ? `<strong>${compactSurname(player.name)}</strong><small class="player-token-stats"><span>${primaryStat}</span><span>シ${system?.rate ?? 0}%/疲${player.fatigue}</span></small>` : `<strong>空き</strong><small>配置</small>`}</button>`; }).join("")}
          </div>
  ...
  private playerCard(player: Player, isSelected: boolean, replacementHint?: "primary" | "secondary") {
    ...
    const suspensionMatches = this.simulation.suspensionMatchesFor(player.id);
    ...
    return `<button data-player="${player.id}" class="squad-card ${isSelected ? "is-selected" : ""} ${replacementHint ? `is-replacement-${replacementHint}` : ""} ${isStarter ? "is-starter" : ""} ${injuryWeeks ? "is-injured" : ""} ${suspensionMatches ? "is-suspended" : ""}" ${(injuryWeeks || suspensionMatches) ? "disabled" : ""}><div><span class="pos-tag">${positionLabel(player.position)}</span></div><h4>${player.name}</h4><p>${player.secondary ? `${positionLabel(player.position)} / ${positionLabel(player.secondary)}` : positionLabel(player.position)}　Lv.${player.level}/${player.ceiling}</p><div class="card-stat"><b>OF${player.attack}/DF${player.defense}</b><span>シ${system.rate}%/疲${player.fatigue}</span></div>${skill ? `<span class="player-skill-chip ${skill.active ? "is-active" : ""}">${skill.short}</span>` : ""}${suspensionMatches ? `<span class="condition-pill danger">停止 / 残${suspensionMatches}試合</span>` : fatiguePill}${playerConditionPill}${replacementHint ? `<span class="replacement-fit ${replacementHint}">${replacementHint === "primary" ? "主適性 / 優先" : "副適性 / 候補"}</span>` : ""}${isStarter ? "<em>STARTING XI</em>" : ""}</button>`;
  }
  ...
  private organizedRosterPlayers() {
    ...
    return filtered.sort((a, b) => {
      ...
      if (this.rosterSalaryFilter === "high") return b.salary - a.salary || ability(b) - ability(a);
      if (this.rosterSalaryFilter === "low") return a.salary - b.salary || ability(b) - ability(a);
  ...
  private teamPage() {
    ...
    const foreignCount = this.simulation.foreignPlayerCount;
    const foreignAlert = foreignCount >= this.simulation.foreignPlayerLimit ? `<section class="roster-capacity-alert tactical-card is-warning"><div><span class="card-kicker">FOREIGN PLAYER LIMIT</span><h3>外国人枠が上限です</h3><p>外国籍選手は ${foreignCount} / ${this.simulation.foreignPlayerLimit} 人。新たに外国籍選手を獲得するには、先に登録中の外国籍選手を整理してください。</p></div><b>FULL</b></section>` : "";
    ...
    const salaryFilters: Array<{ id: SalaryFilter; label: string }> = [{ id: "all", label: "指定なし" }, { id: "high", label: "高い順" }, { id: "low", label: "低い順" }];
    ...
    return `
      ...
      ${capacityAlert}${foreignAlert}
      <section class="roster-organizer tactical-card"><div class="roster-organizer-head"><div><span class="card-kicker">CONTRACT ORGANIZER</span><h3>年俸・契約で整理</h3><p>年俸の並び順と契約条件を組み合わせ、優先して見直す契約を絞り込みます。</p></div><b>${organizedRoster.length}<small> / ${this.simulation.rosterPlayers.length}名</small></b></div><div class="roster-control-set"><span>並び順</span><div>${sortOptions.map((option) => `<button type="button" data-roster-sort="${option.id}" class="${this.rosterSort === option.id ? "is-selected" : ""}">${option.label}</button>`).join("")}</div></div><div class="roster-control-grid"><div class="roster-control-set"><span>年俸順</span><div>${salaryFilters.map((filter) => `<button type="button" data-roster-salary-filter="${filter.id}" class="${this.rosterSalaryFilter === filter.id ? "is-selected" : ""}">${filter.label}</button>`).join("")}</div></div><div class="roster-control-set"><span>契約残年数</span><div>${contractFilters.map((filter) => `<button type="button" data-roster-contract-filter="${filter.id}" class="${this.rosterContractFilter === filter.id ? "is-selected" : ""}">${filter.label}</button>`).join("")}</div></div></div></section>
      <section class="roster-table tactical-card"><div class="table-heading"><div><span class="card-kicker">PLAYER DIRECTORY</span><h3>選手一覧</h3></div><span>${organizedRoster.length ? "カードをタップして詳細を見る" : "条件に一致する選手はいません"}</span></div><div class="table-head"><span>選手情報</span><span>能力 / 年俸 / 契約</span></div>${organizedRoster.length ? organizedRoster.map((player) => { const injuryWeeks = this.simulation.injuryWeeksFor(player.id); const years = player.contractYears ?? 1; return `<button type="button" data-player-detail="${player.id}" class="table-row player-directory-card ${injuryWeeks ? "is-injured" : ""}"><span class="player-directory-info"><strong><i class="pos-tag">${positionLabel(player.position)}</i><span>${player.name}<small>詳細を見る ›</small></span></strong><small class="player-directory-role">${positionLabel(player.position)}${player.secondary ? ` / ${positionLabel(player.secondary)}` : ""}　/　${player.age}歳</small></span><span class="player-directory-summary"><b>${player.position === "GK" ? `GK ${player.gk} / OF${player.attack}` : `OF${player.attack} / DF${player.defense}`}</b><small>${formatMoney(player.salary)} / 年</small><small class="player-directory-contract">残り ${years}年${years <= 1 ? " / 更新優先" : ""}</small></span></button>`; }).join("") : `<div class="roster-filter-empty">現在の年俸・契約条件に一致する選手はいません。条件を「全」に戻して確認してください。</div>`}</section>
  ...
  private playerDetailPanel() {
    ...
    const conditionStatus = this.simulation.conditionStatusFor(player);
    const condition = injuryWeeks ? `負傷中 / 復帰まで ${injuryWeeks}週` : `状態 ${conditionStatus.label} / ${conditionStatus.value} / 疲労 ${player.fatigue}`;
    ...
    const relevantAttributes = player.position === "GK" ? ["gk", "pass", "defense"] : ["CF", "WG", "SH"].includes(player.position) ? ["attack", "dribble", "shoot", "pass"] : ["AM", "CM", "DM"].includes(player.position) ? ["pass", "dribble", "attack", "defense"] : ["defense", "tackle", "block", "interception"];
    const growthAttributes = this.simulation.attributeProgressFor(player).filter((item) => relevantAttributes.includes(item.attribute));
    const mastery = this.simulation.positionMasterySummaryFor(player);
    const system = this.simulation.systemEffectivenessFor(player);
    const systemMastery = this.simulation.systemMasterySummaryFor(player);
    const growthPanel = `<section class="development-profile"><div class="development-profile-head"><span>INDIVIDUAL DEVELOPMENT</span><b>能力XP / ポジション熟練度</b></div><div class="attribute-growth-grid">${growthAttributes.map((item) => `<div class="attribute-growth-meter"><div><span>${item.label}</span><b>${item.value}<small> XP ${item.xp}/100</small></b></div><i><em style="width:${item.progress}%"></em></i></div>`).join("")}</div><div class="position-mastery-grid">${mastery.map((item) => `<div class="position-mastery-meter"><div><span>${item.label}</span><b>${item.totalXp}<small> ${item.levelLabel}</small></b></div><i><em style="width:${item.progress}%"></em></i></div>`).join("")}</div><p>練習で選んだ能力にXPが蓄積し、100XPごとに能力値が+1。実際の起用位置で経験を積むと熟練度が上がります。</p></section><section class="system-intelligence-profile"><div class="system-intelligence-head"><div><span>SYSTEM INTELLIGENCE</span><h3>システム理解力 <b>${system.understanding}</b></h3></div><strong>${system.rate}<small>%</small><em>${system.grade}</em></strong></div><p>${system.note}</p><div class="current-system-mastery"><span>${system.formationLabel} 習熟度</span><b>${system.mastery}</b><i><em style="width:${system.mastery}%"></em></i></div><div class="system-mastery-list">${systemMastery.map((item) => `<span class="${item.formationId === system.formationId ? "is-current" : ""}"><b>${item.formationLabel}</b><em>${item.mastery}</em><small>発揮 ${item.rate}%</small></span>`).join("")}</div><small>理解力は戦術を吸収する基礎能力。各布陣の習熟度は、そのシステムで練習・出場すると上がります。</small></section>`;
    return `<div class="player-detail-overlay" role="dialog" aria-modal="true" aria-label="${player.name}の選手詳細"><section class="player-detail-panel"><header><div><span>PLAYER PROFILE</span><h2>${player.name}</h2><p>${role}</p></div><button type="button" data-close-player-detail aria-label="選手詳細を閉じる">×</button></header><div class="detail-identity"><span class="pos-tag">${positionLabel(player.position)}</span><div><b>${player.age}<small>歳</small></b><p>年齢</p></div><div><b>LV.${player.level}<small> / ${player.ceiling}</small></b><p>現在レベル / 上限</p></div></div><section class="detail-ability"><h3>OFFENSE / 攻撃能力</h3><div><span>OF</span><b>${player.attack}</b><i><em style="width:${player.attack}%"></em></i></div><div><span>ドリブル</span><b>${player.dribble}</b><i><em style="width:${player.dribble}%"></em></i></div><div><span>パス</span><b>${player.pass}</b><i><em style="width:${player.pass}%"></em></i></div><div><span>シュート</span><b>${player.shoot}</b><i><em style="width:${player.shoot}%"></em></i></div><h3 class="defense-heading">DEFENSE / 守備能力</h3><div><span>DF</span><b>${player.defense}</b><i class="blue"><em style="width:${player.defense}%"></em></i></div><div><span>タックル</span><b>${player.tackle}</b><i class="blue"><em style="width:${player.tackle}%"></em></i></div><div><span>ブロック</span><b>${player.block}</b><i class="blue"><em style="width:${player.block}%"></em></i></div><div><span>パスカット</span><b>${player.interception}</b><i class="blue"><em style="width:${player.interception}%"></em></i></div>${player.position === "GK" ? `<h3 class="keeper-heading">GOALKEEPING</h3><div><span>GK</span><b>${player.gk ?? 0}</b><i class="gold"><em style="width:${player.gk ?? 0}%"></em></i></div>` : ""}</section>${growthPanel}${cfStyleControl}<section class="detail-note"><span>CONDITION / CONTRACT</span><b class="detail-condition-status ${conditionStatus.tone}" aria-label="コンディション ${condition}"><span class="condition-icon" aria-hidden="true">${conditionStatus.tone === "good" ? "↑" : conditionStatus.tone === "bad" ? "↓" : "→"}</span>${condition}</b><i class="detail-condition-meter"><em style="width:${conditionStatus.value}%"></em></i><small>${conditionStatus.note}</small><p>年俸 ${formatMoney(player.salary)}　/　勝利 ${player.winBonus ? formatMoney(player.winBonus) : "なし"}　/　出場 ${player.appearanceBonus ? formatMoney(player.appearanceBonus) : "なし"}　/　得点 ${player.goalBonus ? formatMoney(player.goalBonus) : "なし"}　/　残り ${player.contractYears ?? 1}年</p></section><button type="button" class="detail-close-button" data-close-player-detail>選手一覧へ戻る</button></section></div>`;
  }
  ...
  private marketPage() {
    ...
    const foreignCount = this.simulation.foreignPlayerCount;
    const candidateNationality = candidate ? this.simulation.foreignPlayerStatusFor(candidate) : null;
    const foreignMarketAlert = foreignCount >= this.simulation.foreignPlayerLimit ? `<section class="roster-capacity-alert tactical-card is-warning"><div><span class="card-kicker">FOREIGN PLAYER LIMIT</span><h3>外国人枠 ${foreignCount} / ${this.simulation.foreignPlayerLimit}</h3><p>外国籍候補を獲得するには、先に外国籍選手の登録枠を空ける必要があります。</p></div><button type="button" data-nav="team" class="ghost-action">チームで整理 <b>→</b></button></section>` : "";
    const marketUpdateBanner = marketUpdate ? `<section class="market-update-alert tactical-card"><div><span class="card-kicker">MARKET INBOX / WEEK ${marketUpdate.week}</span><h3>市場更新のお知らせ</h3><p>${marketUpdate.requestedPositions.length ? `希望ポジション（${marketUpdate.requestedPositions.map(positionLabel).join(" / ")}）に一致する候補です。` : "3節ごとの市場更新で、新しい獲得候補が届きました。"}</p><div class="market-update-players">${marketUpdate.candidates.length ? marketUpdate.candidates.map((player) => `<span><b>${player.name}</b>${positionLabel(player.position)}${player.secondary ? ` / ${positionLabel(player.secondary)}` : ""}</span>`).join("") : "<span>現在の希望条件に一致する候補はいません。</span>"}</div><div class="market-update-players sale-notice">${marketUpdate.saleOffers?.length ? marketUpdate.saleOffers.map((offer) => { const player = this.simulation.rosterPlayers.find((item) => item.id === offer.playerId); return player ? `<span><b>売却オファー</b>${player.name} / ${formatMoney(offer.proposedFee)}</span>` : ""; }).join("") : "<span>今回の売却オファーはありません。</span>"}</div></div><button type="button" data-action="dismiss-market-update" class="ghost-action">通知を確認 <b>✓</b></button></section>` : "";
    ...
    const status = negotiation.stage === "scouting" ? "SCOUTED" : negotiation.stage === "pending" ? "COUNTER OFFER" : "FEE AGREED";
    const action = negotiation.stage === "scouting" ? `<div class="negotiation-choice-list"><button data-action="negotiate-recruit" data-offer-tier="low" class="ghost-action"><strong>低い金額</strong><small>${formatMoney(Math.round(negotiation.openingOffer))} / 強気の交渉</small></button><button data-action="negotiate-recruit" data-offer-tier="fair" class="primary-action"><strong>妥当な金額</strong><small>${formatMoney(negotiation.counterOffer)} / 標準的な交渉</small></button><button data-action="negotiate-recruit" data-offer-tier="high" class="ghost-action"><strong>高めの金額</strong><small>${formatMoney(Math.round(negotiation.counterOffer * 1.18))} / 成立を優先</small></button></div>` : negotiation.stage === "pending" ? `<div class="negotiation-hold-actions"><button data-action="accept-recruit-hold" class="primary-action">追加条件を受け入れる <b>✓</b></button><button data-action="adjust-recruit-hold" class="ghost-action">条件を調整して再提示 <b>↗</b></button><button data-action="withdraw-recruit" class="ghost-action">今回は撤退</button></div>` : negotiation.stage === "agreed" ? `<button data-action="recruit" class="primary-action">${negotiation.contractYears ?? 3}年契約を締結 <b>›</b></button>` : `<span class="contract-clear">交渉終了。次回の市場更新を待ってください。</span>`;
    const dealCopy = negotiation.stage === "scouting" ? "提示額を選びます。低い金額は決裂、高めの金額は成立しやすくなります。" : negotiation.stage === "pending" ? `契約保留：${negotiation.holdReason ?? "追加条件の確認が必要です。"}` : negotiation.stage === "failed" ? "今回の交渉は終了しました。" : `契約成功：移籍金 ${formatMoney(negotiation.agreedFee ?? negotiation.counterOffer)} / 年俸 ${formatMoney(negotiation.proposedSalary ?? candidate.salary)} / ${negotiation.contractYears ?? 3}年`;
    return `
      ${this.pageHeading("TRANSFER WINDOW", "市場で才能を見つける", "戦術に足りない最後のピースを、今ここで。")}
      ${capacityAlert}${foreignMarketAlert}
      ${marketUpdateBanner}
      ...
      <section class="market-feature tactical-card"><div class="market-image" style="background-image:linear-gradient(0deg,rgba(4,20,12,.82),rgba(4,20,12,.04)),url('${assets.scoutCard}')"><span>SCOUT REPORT</span></div><div class="market-details"><span class="card-kicker">PRIORITY TARGET / ${status}</span><h2>${candidate.name} <em>${positionLabel(candidate.position)}${candidate.secondary ? ` / ${positionLabel(candidate.secondary)}` : ""}</em></h2><p class="nationality-badge">${candidateNationality?.foreign ? `外国籍 / ${candidateNationality.label}` : "日本籍"}　外国人枠 ${foreignCount} / ${this.simulation.foreignPlayerLimit}</p><p>${playerAssessmentFor(candidate)}</p><div class="market-stats"><span><b>${candidate.position === "GK" ? `GK ${candidate.gk}` : `OF ${candidate.attack}`}</b>OFFENSE</span><span><b>DF ${candidate.defense}</b>DEFENSE</span><span><b>${candidate.age}</b>AGE</span><span><b>Lv.${candidate.level}/${candidate.ceiling}</b>GROWTH</span></div>${candidateNeed ? `<div class="candidate-need-match"><span>CANDIDATE NEED MATCH</span><b>${candidateNeed.note}</b><small>${positionLabel(candidateNeed.priority.position)}補強優先度 ${candidateNeed.score} / 稼働 ${candidateNeed.priority.available}人 / 戦力平均 ${candidateNeed.priority.averagePower}</small></div>` : ""}${fit ? `<section class="tactical-fit-report"><div class="fit-report-head"><div><span>戦術適合 / ${fit.grade}</span><h3>現行プランとの戦術適合</h3><p>${fit.formationLabel} / ${fit.mentalityLabel} / ${fit.playingStyleLabel} を基準に再評価。</p></div><strong>${fit.score}<small>/100</small></strong></div><div class="fit-breakdown"><span><b style="width:${fit.formation}%"></b><i>フォーメーション</i><em>${fit.formation}</em></span><span><b style="width:${fit.mentality}%"></b><i>攻守意識</i><em>${fit.mentality}</em></span><span><b style="width:${fit.playingStyle}%"></b><i>プレースタイル</i><em>${fit.playingStyle}</em></span><span><b style="width:${fit.chemistry}%"></b><i>連携</i><em>${fit.chemistry}</em></span></div><ul class="fit-strengths">${fit.strengths.map((strength) => `<li>${strength}</li>`).join("")}</ul><p class="fit-concern"><b>SCOUT NOTE</b>${fit.concern}</p></section>` : ""}<div class="market-deal-strip"><span>SCOUT REPORT / ${this.simulation.marketQualityNote}</span><b>${negotiation.stage === "agreed" ? formatMoney(negotiation.agreedFee ?? negotiation.counterOffer) : negotiation.stage === "pending" ? formatMoney(negotiation.counterOffer) : formatMoney(negotiation.openingOffer)}</b><small>${dealCopy}</small></div><div class="market-footer"><p>年俸 <b>${formatMoney(negotiation.proposedSalary ?? candidate.salary)}</b>　/　契約期間 <b>${negotiation.contractYears ?? 3}年</b>　/　追加条件 <b>${negotiation.appearanceGuarantee ? "出場機会保証" : "出来高条件あり"}</b>　/　候補更新 <b>あと${this.simulation.marketRefreshIn}週</b></p>${action}</div></div></section>
    `;
  }
  ...
  private highlightsTimeline(highlights: NonNullable<ReturnType<ClubSimulation["advanceWeek"]>>["highlights"], throughHalf: boolean, visibleCount?: number) {
    ...
    const line = (item: MatchHighlight) => `<article class="highlight-item ${item.team} ${item.kind}"><b>${String(item.minute).padStart(2, "0")}′</b><span>${item.kind === "goal" ? "GOAL" : item.kind === "sequence" ? "RALLY" : item.kind === "action" ? "PLAY" : item.kind === "injury" ? "MED" : item.kind === "card" ? "CARD" : item.kind === "tactic" ? "PLAN" : item.kind === "halftime" ? "HT" : "•"}</span><p>${item.text}</p></article>`;
  ...
  private liveCommentaryStatus(result: NonNullable<ReturnType<ClubSimulation["advanceWeek"]>>, phase: "first-half" | "second-half") {
    ...
    return `<section class="live-commentary-status"><div><span><i></i>LIVE COMMENTARY</span><b>${label}の実況を受信中</b><small>${this.commentaryVisibleCount} / ${total} 行目　次のコメントまで3秒</small></div></section>`;
  }

  private matchStatsPanel(result: NonNullable<ReturnType<ClubSimulation["advanceWeek"]>>) {
    const home = result.stats.orbit;
    const away = result.stats.opponent;
    const rows: Array<[string, keyof typeof home]> = [["支配率", "possession"], ["シュート", "shots"], ["枠内シュート", "shotsOnTarget"], ["決定機", "bigChances"], ["パス", "passes"], ["パス成功率", "passAccuracy"], ["CK", "corners"], ["ファウル", "fouls"], ["オフサイド", "offsides"], ["セーブ", "saves"]];
    const formatValue = (key: keyof typeof home, value: number) => key === "possession" || key === "passAccuracy" ? `${value}%` : value.toLocaleString();
    const barWidth = (key: keyof typeof home, value: number, opponentValue: number) => key === "possession" || key === "passAccuracy" ? value : Math.round((value / Math.max(value, opponentValue, 1)) * 100);
    return `<section class="match-stats-card"><div class="match-stats-head"><div><span>FULL TIME STATS</span><b>試合スタッツ</b></div><small>試合中の攻防を数値で振り返る</small></div><div class="match-stats-teams"><strong>${escapeHtml(this.simulation.clubNameValue)}</strong><span>HOME / ORBIT</span><strong>${escapeHtml(result.opponent)}</strong></div><div class="match-stats-rows">${rows.map(([label, key]) => { const homeValue = home[key]; const awayValue = away[key]; const homeLead = homeValue > awayValue ? "is-lead" : ""; const awayLead = awayValue > homeValue ? "is-lead" : ""; return `<div class="match-stat-row"><div class="match-stat-value ${homeLead}">${formatValue(key, homeValue)}</div><div class="match-stat-middle"><span>${label}</span><i><em class="home" style="width:${barWidth(key, homeValue, awayValue)}%"></em><em class="away" style="width:${barWidth(key, awayValue, homeValue)}%"></em></i></div><div class="match-stat-value ${awayLead}">${formatValue(key, awayValue)}</div></div>`; }).join("")}</div></section>`;
  }
