...
export class GameUI {
  ...
  private markingForOpponent(opponentPlayer: OpponentPlayer, opponentSlot: Slot): MarkingAssessment | null {
    ...
    const manualPairing = manualPlayerId ? candidates.find((candidate) => candidate.player.id === manualPlayerId && canMarkOpponent(opponentPlayer.position, candidate.player.position)) : undefined;
    const pairing = manualPairing ?? [...candidates].sort((a, b) => {
      const aRank = markingDefenderRank(opponentPlayer.position, a.player.position) ?? 99;
      const bRank = markingDefenderRank(opponentPlayer.position, b.player.position) ?? 99;
      return aRank - bRank || ((a.slot.x - opponentSlot.x) ** 2 + (a.slot.y - opponentSlot.y) ** 2) - ((b.slot.x - opponentSlot.x) ** 2 + (b.slot.y - opponentSlot.y) ** 2);
    })[0];
  ...
  private decorateManualMarkControls() {
    ...
    content.insertAdjacentHTML("beforebegin", `<section class="manual-mark-console"><header><div><span>MANUAL MARKING</span><b>守備担当を手動指定</b></div><div><i>${Object.keys(manualAssignments).length} / ${starters.length} 指定</i><button type="button" data-mark-clear ${Object.keys(manualAssignments).length ? "" : "disabled"}>自動へ戻す</button></div></header><p>CB・SB・DH・CH・SHの選手札を<strong>相手の前線・中盤の攻撃選手へドラッグ</strong>してください。GK・CB・SBは個別マーク対象外です。スマホでは、選手札をタップしてから相手をタップします。</p><div class="manual-mark-sources">${starters.map(({ slot, player }) => `<button type="button" draggable="true" data-mark-source="${player.id}" class="manual-mark-source ${this.manualMarkSourceId === player.id ? "is-selected" : ""} ${assignedPlayerIds.has(player.id) ? "is-assigned" : ""}" aria-pressed="${this.manualMarkSourceId === player.id}" aria-label="${player.name}を守備担当に選択、またはドラッグ"><i>${positionLabel(slot.label)}</i><b>${player.name}</b><small>${assignedPlayerIds.has(player.id) ? "担当済" : "ドラッグ"}</small></button>`).join("")}</div></section>`);
  }
  ...
  private decorateOpponentMarkings() {
    ...
    if (pitchHead) pitchHead.insertAdjacentHTML("afterend", `<div class="marking-legend"><span><i class="advantage"></i>有利</span><span><i class="even"></i>拮抗</span><span><i class="caution"></i>警戒</span><span><i class="not-required"></i>対象外</span><b>攻撃・中盤の相手は守備担当、GK・CB・SBはゾーン対応</b></div>`);
