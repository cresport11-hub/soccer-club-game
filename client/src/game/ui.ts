...
export class GameUI {
  ...
  private teamPage() {
    ...
    return `
      ...
      <section class="roster-table tactical-card"><div class="table-heading"><div><span class="card-kicker">PLAYER DIRECTORY</span><h3>選手一覧</h3></div><span>${organizedRoster.length ? "カードをタップして詳細を見る" : "条件に一致する選手はいません"}</span></div><div class="table-head"><span>選手</span><span>役割 / 契約</span><span>能力</span><span>状態</span><span>年齢</span><span>年俸</span></div>${organizedRoster.length ? organizedRoster.map((player) => { const injuryWeeks = this.simulation.injuryWeeksFor(player.id); const years = player.contractYears ?? 1; return `<button type="button" data-player-detail="${player.id}" class="table-row player-directory-card ${injuryWeeks ? "is-injured" : ""}"><strong><i class="pos-tag">${positionLabel(player.position)}</i><span>${player.name}<small>詳細を見る ›</small></span></strong><span>${positionLabel(player.position)}${player.secondary ? ` / ${positionLabel(player.secondary)}` : ""}<small class="roster-contract-years">残り ${years}年${years <= 1 ? " / 更新優先" : ""}</small></span><span>${player.position === "GK" ? `GK ${player.gk} / OF${player.attack}` : `OF${player.attack} / DF${player.defense}`}</span><span><b class="condition-label ${this.simulation.conditionStatusFor(player).tone}" aria-label="コンディション ${this.simulation.conditionStatusFor(player).label} ${this.simulation.conditionFor(player)}"><span class="condition-icon" aria-hidden="true">${this.simulation.conditionStatusFor(player).tone === "good" ? "↑" : this.simulation.conditionStatusFor(player).tone === "bad" ? "↓" : "→"}</span>${this.simulation.conditionStatusFor(player).label} ${this.simulation.conditionFor(player)}</b></span><span>${player.age}歳</span><span>${formatMoney(player.salary)}</span></button>`; }).join("") : `<div class="roster-filter-empty">現在の年俸・契約条件に一致する選手はいません。条件を「全」に戻して確認してください。</div>`}</section>
